...
2. [`supabase/migrations/001_init.sql`](supabase/migrations/001_init.sql) の内容をすべて貼り付けます。
3. 上から最後まで実行します。

テーブル、インデックス、Row Level Security（RLS）とユーザーごとのアクセスポリシーが作成されます。
...
ホームの「会話する」が学習の中心です。1日の目安は次の流れです。

1. 単語カードを10枚練習する
2. 言語ペアレントと1回会話する
3. 会話で出会った新しい言葉を必要に応じてカードへ残す
