...
3. 006(`006_chunks.sql`)は表現の台帳と今日の狙い、007(`007_curriculum.sql`)は会話レッスンの進み具合を作ります。未適用だとアプリに実行の案内が出ます(練習は続けられます)。
...
出会い 8 回・自分で言えた 3 回・2 つ以上の場面で「身についた」と数えます。1日の目安は次の順番です(約 40 分)。

1. 会話レッスン(20〜25 分): 用意された 10 本(旅行・生活の場面)を順番に。自然な声で、前のレッスンの復習も入ります。終わったら「言えた割合」を自己申告し、8 割以上で次へ
2. 2 語で言う(3 分): 動詞 25 語から選んで、日本語のお題を 2 語で。4 問 45 秒。慣れたら 3 語・4 語へ(英語のみ)
3. 型を回す(6 分): 狙いの型と句動詞から
4. ピースをつなぐ(6 分): ピース(I can 〜)にかたまりをはめ、ラージピース(I didn't know that 〜)に文をつなぐ(英語のみ)
5. 単語カード(5 分): 狙いのカードを先に
6. 聞いて書く(5 分): 狙いを含む文を先に
7. 聞き流し(4 分): 保存した会話 3 本を文字なしで
8. ChatGPT で会話(10 分): プロンプトをコピーして貼り、終わったら「まとめ」と送って使えた表現を記録する

会話レッスンは一覧から任意の 1 本も選べます。自由な場面のレッスン(Gemini で生成)は補助として残っています。
...
## 7.5 同梱レッスンの音声を作る(開発者向け)

会話レッスンの音声は `public/lessons/` に同梱した mp3 で、PC で一度だけ作ります(CI では作りません)。

1. `.env` に `GEMINI_API_KEY=` を追加します(このスクリプトだけが使い、アプリには渡りません)
2. `npm run lessons:audio -- --preview-voices` でナレーター候補の声を聴き比べ、必要なら `--narrator <名前>` で指定します
3. `npm run lessons:audio -- --lang en --dry-run` で件数を確認し、`npm run lessons:audio -- --lang en` で合成します(1 本 ≈ 19 リクエスト)
4. 無料枠の 1 日の上限に当たると止まります。翌日そのまま再実行すれば続きから作ります。`npm run lessons:check` で進み具合を見られます
